...
User = get_user_model()
POSTS_PER_PAGE: int = 10  # настройку можно вынести в settings.POSTS_ON_INDEX
...
def _get_posts_queryset(
    *,
    ...
    """Возвращает QS постов с нужными фильтрами, select_related и annotate."""
    qs = base_qs
    if filter_published:
        qs = qs.filter(
            is_published=True,
...
def post_detail(request, post_id):
    """Показывает пост + форму комментария. Неопубликованное видит только автор."""
    post = get_object_or_404(Post, pk=post_id)
    if post.author != request.user:
        post = get_object_or_404(_get_posts_queryset(select_related_fields=False), pk=post_id)
...
def profile(request, username):
    ...
    posts = _get_posts_queryset(base_qs=profile_user.posts.filter(category__isnull=False), filter_published=request.user != profile_user)
    page_obj = _paginate(request, posts)
...
def registration(request):
    ...
    URL: /auth/registration/  Имя: blog:registration
    """
    form = UserCreationForm(request.POST or None)
    if request.method == 'POST' and form.is_valid():
        form.save()
        return redirect('login')      # стандартный auth.views.LoginView
    return render(request, 'registration/registration_form.html', {'form': form})


@login_required
def edit_profile_self(request):
    """Редактирование собственного профиля по URL /profile/edit/."""
